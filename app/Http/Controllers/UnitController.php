...
class UnitController extends Controller
{
    public function getInfo($id)
    {
        ...
        if ($unit) {
            ...
            foreach ($peoples as $pKey => $pValue) {
                $peoples[$pKey]['birth_date'] = date('d/m/Y', strtotime($pValue['birth_date']));
            }
    ...
    public function addPerson($id, Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            $newPerson->name = $name;
            $newPerson->birth_date = $birthdate;
            $newPerson->save();
        } else {
    ...
    public function addVehicle($id, Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            $newVehcile = new UnitVehicle();
            $newVehcile->id_unit = $id;
            $newVehcile->title = $title;
            $newVehcile->color = $color;
            $newVehcile->plate = $plate;
            $newVehcile->save();
        } else {
            $array['error'] = $validator->errors()->first();
            return $array;
